...
class stock_quant(osv.osv):
    ...
    _columns = {
        'supplier_seiral_no': fields.related(
            'lot_id',
            'supplier_seiral_no',
            type='char',
        ...
        'production_date': fields.related(
            'lot_id',
            'production_date',
            type='date',
            relation='stock.production.lot',
        ...
        'reinspec_number': fields.related(
            'lot_id',
            'reinspec_number',
            type='char',
            relation='stock.production.lot',
        ...
        'production_batch': fields.related(
            'lot_id',
            'production_batch',
            type='char',
            relation='stock.production.lot',
        ...
        'structure_level': fields.related(
            'lot_id',
            'structure_level',
            type='char',
            relation='stock.production.lot',
        ...
        'concession_or_replace_number': fields.related(
            'lot_id',
            'concession_or_replace_number',
            type='char',
            relation='stock.production.lot',
            string='让步/代用单号',
            readonly=True,
        ...
        'meterial_plan_status': fields.related(
            'lot_id',
            'meterial_plan_status',
            type='char',
            relation='stock.production.lot',
            string='物资计划状态',
            readonly=True,
        ...
        'meterial_quality_status': fields.related(
            'lot_id',
            'meterial_quality_status',
            type='char',
            relation='stock.production.lot',
            string='物资计划状态',
            readonly=True,
        ...
        'quality_info_number': fields.related(
            'lot_id',
            'quality_info_number',
            type='char',
            relation='stock.production.lot',
        ...
        'special_requirement': fields.related(
            'lot_id',
            'special_requirement',
            type='char',
            relation='stock.production.lot',
            string='特殊要求',readonly=True,
            select=True,
            store=True
        ),
        ...
        'product_category': fields.related(
            'lot_id',
            'product_category',
            type='char',
            relation='stock.production.lot',
...
class stock_move(osv.osv):
    ...
    def create_and_assign_lot(self, cr, uid, product_id,name=False, context=None):
        ...
        if not new_lot_id:
            new_lot_id = self.pool.get('stock.production.lot').create(
        ...
        if picking_id:
            if product_id and not restrict_lot_id and picking_type_id:
                ...
                if self.pool.get('stock.picking.type').browse(
                        cr, uid,
                        picking_type_id, context=context)['auto_create_lot']:
                    val = self.create_and_assign_lot(
    ...
    def onchange_product_id(
        ...
        vals = {}
        if prod_id:
            vals = super(stock_move, self).onchange_product_id(
                cr, uid, ids, prod_id, loc_id, loc_dest_id, partner_id)
...
class stock_picking(osv.osv):
    ...
    def _state_get(self, cr, uid, ids, field_name, arg, context=None):
        res = super(stock_picking, self)._state_get(
        ...
        for picking in self.browse(cr, uid, ids, context=context):
            scan_operation_type = \
                picking.picking_type_id.scan_operation_type or '1'
    ...
    def open_barcode_interface(self, cr, uid, picking_ids, context=None):
        final_url = \
            "/barcode/web/?&debug#action=stock.ui&picking_id=" + \
            str(picking_ids[0])
        return {
            'type': 'ir.actions.act_url',
            'url':final_url,
            'target': '_blank',
            'style':'height=500,width=600,toolbar=0,menubar=0'
        }
    ...
    def _prepare_pack_ops(
        ...
        def _picking_putaway_apply(product):
            ...
            if product_putaway_strats.get(product.id):
                ...
                location = self.pool.get('stock.location')\
                    ...
                    cr, uid, picking.location_dest_id,
        ...
        for move in picking.move_lines:
            ...
            if move.product_uom.id != move.product_id.uom_id.id and \
                move.product_uom.factor > product_uom[move.product_id.id].factor:
        ...
        for pack in top_lvl_packages:
            ...
            # remove the quants inside the package so that they are excluded
        ...
        for product, qty in forced_qties.items():
            ...
            key = (
                ...
                product[1].id,
                picking.owner_id.id,
                picking.location_id.id if picking.location_id.id else picking.picking_type_id.default_location_src_id.id,
        ...
        # prevals var holds the operations in order to create them in the same
        # order than the picking stock moves if possible
    ...
    def do_prepare_partial(self, cr, uid, picking_ids, context=None):
        ...
        #used to avoid recomputing the remaining quantities at each new pack
        # operation created
        ctx = context.copy()
        ctx['no_recompute'] = True

        picking = self.pool.get('stock.picking').browse(
            cr, uid, picking_ids[0], context=context)
        ...
        existing_package_ids = pack_operation_obj.search(
            cr, uid,
            [('picking_id', 'in', picking_ids),
             ('processed', '=', 'false')],
            context=context)
        if existing_package_ids:
            pack_operation_obj.unlink(
                cr, uid, existing_package_ids, context)
    ...
    def process_product_id_from_ui(
            self, cr, uid, picking_id, product_id, op_id, increment=True,
            context=None
    ):
        return
    ...
    def process_barcode_from_ui(
            self, cr, uid, picking_id, barcode_str, visible_op_ids,
            context=None):
        answer = super(stock_picking, self).process_barcode_from_ui(
            cr, uid, picking_id, barcode_str, visible_op_ids, context=context)
...
class stock_pack_operation(osv.osv):
    ...
    def check_operation(
            self, cr, uid, picking_id, product_id, lot_id=False,
            increment=True, context=None
    ):
        product = self.pool.get("product.product").browse(
            cr, uid, product_id, context=context)
        picking = self.pool.get("stock.picking").browse(
        ...
        if scan_operation_type in ['2', '4'] and lot_id > 0:#扫描时核对产品
            ...
            domain = [
                ...
                ['lot_id', '=', lot], ['product_qty', '>', 0]
            ]
            group_res = self.read_group(
                cr, uid, domain, ['product_qty', 'qty_done', 'lot_id'],
                ['lot_id'])
        ...
        if increment and product.uom_type == "one":
            operations = self.search_read(
                cr, uid, [('picking_id', '=', picking_id),
                          ...
                          ('lot_id', '=', lot_id),
                          ('qty_done', '>', 0)],
                ['qty_done'], context=context)
            scaned_qty = sum([x['qty_done'] for x in operations]) or 0.0
            if scaned_qty >=1:
                raise osv.except_osv(
                    _('Error'),
                    u'产品[%s]一个序列号只能扫描一个数量!' % product.name)

    def update_origin_operation_qty(
            self, cr, uid, picking_id, product_id, increment=True,
            context=None):
        #修改原出库的数量
        origin_operation_ids = False
        origin_operation_ids = self.search(
            cr, uid, [('picking_id', '=', picking_id),
                      ('product_id', '=', product_id),
                      ('lot_id', '=', False),
                      ('product_qty', '>', 0)], context=context)
        if origin_operation_ids:
            origin_operation_id = origin_operation_ids[0]
            op_obj = self.browse(
                cr, uid, origin_operation_id, context=context)
            ...
            if qty == 0 and op_obj.qty_done == 0:
                ...
                self.write(
                    cr, uid, [origin_operation_id],
                    {'product_qty': qty}, context=context)

    def _search_and_increment(
            self, cr, uid, picking_id, domain, filter_visible=False,
            visible_op_ids=False, increment=True, context=None):
        ...
        self.check_operation(
            cr, uid, picking_id, product_id, lot_id, increment, context=None)

        #if current_package_id is given in the context, we increase the number
        # of items in this package
        package_clause = [
            ('result_package_id',
             '=',
            context.get('current_package_id', False))
        ]
        ...
        if todo_operation_ids:
            #existing operation found for the given domain and
            # picking => increment its quantity
            ...
            context.update({'increment':increment, 'no_recompute': True})
            # 不加no_recompute时，速度很慢
            vals = {'qty_done': qty}
            if scan_operation_type in ['2', '3']:
                vals.update({'product_qty': qty})
            self.write(cr, uid, [operation_id], vals, context=context)
        else:
            #no existing operation found for the given domain and
            # picking => create a new one
            ...
            values = {
                'picking_id': picking_id,
                'product_qty': 0,
                #*****modifed by xie xiaopeng 2015.04.10
                # for when picking not have move items***/
                #'location_id': picking.location_id.id, 
                #'location_dest_id': picking.location_dest_id.id,
            ...
            for key in domain:
                ...
                uom_id = False
                if var_name == 'product_id':
                    uom_id = self.pool.get('product.product').browse(
                        cr, uid, value, context=context).uom_id.id
            ...
            if lot_id > 0:#when scan lot no
                self.update_origin_operation_qty(
                    cr, uid, picking_id, product_id, increment,
                    context=context)
        return operation_id
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'qty_done' in vals:
            ...
            self.update_origin_operation_qty(
                cr, uid, op_obj.picking_id.id, op_obj.product_id.id,
                increment, context=context)

        res = super(stock_pack_operation, self).write(
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        super(stock_pack_operation, self).unlink(
        ...
        if move_operation_link_ids:
            move_operation_link_obj.unlink(
                cr, uid, move_operation_link_ids, context=context)

    def update_operation_product_qty_by_move(
            self, cr, uid, move_ids, context=None):
        #update operation prucuct_qty
        vals = {}
        if move_ids:
            for move in self.pool.get('stock.move').browse(
                    cr, uid, move_ids,context):
                all_opteration_ids = self.pool.get(
                    'stock.move.operation.link').search_read(
                    cr, uid, [('move_id', '=', move.id)], ['operation_id'])
                all_opteration_ids = all_opteration_ids and all_opteration_ids[0]['operation_id'][0] or False
                if all_opteration_ids:
                    all_opteration_ids = self.search(
                        cr, uid, [
                            ('id', 'in', [all_opteration_ids]),
                            ('lot_id', '=', False)], context=context)
                    if all_opteration_ids:
                        ...
                        super(stock_pack_operation, self).write(
                            cr, uid, processed_ids, vals, context=context)
...
class stock_picking_type(osv.osv):
    ...
    _columns = {
        ...
        'partial_delivey': fields.boolean(
            u'可部分扫描',
            help=u'发货明细表的发货字段打勾时, 才能出现在扫描页面.'
        ),
    }
...
class mrp_bom_line2(osv.osv):
    ...
    _columns = {
        'type': fields.selection(
            ...
            'BoM Line Type',
            required=True,
            help="Phantom: this product line will not appear in the raw"
                 "materials of manufacturing orders,"
                 "it will be directly replaced by the raw materials"
                 "of its own BoM, without triggering"
        ...
        'product_uos': fields.many2one(
            'product.uom',
            'Product UOS',
            help="Product UOS (Unit of Sale) is the unit of measurement"
                 "for the invoicing and promotion of stock."
        ),
        ...
        'date_start': fields.date(
            'Valid From',
            help="Validity of component. Keep empty if it's always valid."
        ),
        ...
        'routing_id': fields.many2one(
            ...
            help="The list of operations (list of work centers) to produce"
                 ...
                 "on work centers based on production planning."
        ),
        ...
        'child_line_ids': fields.function(
            _get_child_bom_lines,
    ...
    def _get_uom_id(self, cr, uid, *args):
        return self.pool["product.uom"].search(
            cr, uid, [], limit=1, order='id')[0]
    _defaults = {
    ...
    _sql_constraints = [
        (
            'bom_qty_zero',
             ...
             'You should install the mrp_byproduct module if you want to'
             'manage extra products on BoMs !'
         ),
    ]

    def onchange_uom(
            self, cr, uid, ids, product_id, product_uom, context=None):
        res = {'value': {}}
        ...
        uom = self.pool.get('product.uom').browse(
            cr, uid, product_uom, context=context)
        if uom.category_id.id != product.uom_id.category_id.id:
            res['warning'] = {
                'title': _('Warning'),
                'message': _(
                    'The Product Unit of Measure you chose has a'
    ...
    def onchange_product_id(
            self, cr, uid, ids, product_id, product_qty=0, context=None):
