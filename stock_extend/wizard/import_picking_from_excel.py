...
class import_picking_from_excel_wizard(osv.osv_memory):
    ...
    def import_picking_from_excel(self, cr, uid, ids, context=None):
        ...
        # try:
        picking_ids = self.pool.get('incoming.shipment.converter')\
            .create_picking_from_excel_file_path(
...
class incomingShipmentConverter(osv.osv):
    ...
    def create_picking_from_excel_file_path(self, file_path):
        picking_importer = incoming_shipment_excel_importer(
            file_path=file_path)
    ...
    def convert_odoo_value_from_value(self, excel_val):
        ...
        ship_val_after_lines_update = self.update_shipping_move_lines(
            ship_val_after_header_update)
    ...
    def update_shipping_header(self, excel_val):
        ...
        ship_val_after_header_update['picking_type_id'] =\
            self.get_picking_type_id()
        ship_val_after_header_update['partner_id'] =\
            self.find_create_partner_id(excel_val['partner_id'])
    ...
    def update_shipping_move_lines(self, ship_val_after_header_update):
        ...
        for excel_line in excel_move_lines:
            updated_move_lines.append(self.update_one_move_line(excel_line))
    ...
    def update_one_move_line(self, excel_line):
        ...
        updated_line[2]['product_id'] = self.find_create_product_id_from_sku(
            excel_line[2]['product_id'],
            excel_line[2]['product_name'])
        updated_line[2]['product_uom'] = self.find_create_uom_from_name(
            excel_line[2]['product_uom'])
        updated_line[2]['location_id'] =\
            self.find_location_src_dest_from_picking_type()[0]
        updated_line[2]['location_dest_id'] =\
            self.find_location_src_dest_from_picking_type()[1]
        # ***add by xie xiaopeng***#
        updated_line[2]['restrict_lot_id'] =\
            self.create_find_serial_number(updated_line)
    ...
    def update_one_pack_line(self, move):
        ...
        updated_pack_line[2]['location_id'] =\
            self.find_location_src_dest_from_picking_type()[0]
        updated_pack_line[2]['location_dest_id'] =\
            self.find_location_src_dest_from_picking_type()[1]
        # ***add by xie xiaopeng***#
    ...
    def create_find_serial_number(self, move):
        ...
        if move[2]['product_uom_qty'] > 1 and product.uom_type == "one":
            raise osv.except_osv(
                u'错误:[%s]数量类型为[个]时,数量不能大于1,请拆分成每行数量为1.' %\
                move[2]['name'], '')
        ...
        lot_id = lot_obj.search(
            [('supplier_seiral_no', '=', supplier_seiral_no),
             ('product_id.default_code', '=', default_code)], limit=1)
        if not lot_id:
    ...
    def find_create_product_id_from_sku(self, product_sku, product_name):
        ...
        if not product_id.id or product_id.name != product_name:
            ...
            raise osv.except_osv(
                u'错误', u"没有找到产品定义码为%s产品或该定义码的产品名称有误!" % product_sku)
    ...
    def find_location_src_dest_from_picking_type(self):
        inchoming_type_id = self.env.ref('stock.picking_type_in')
        return inchoming_type_id.default_location_src_id.id,\
               inchoming_type_id.default_location_dest_id.id
