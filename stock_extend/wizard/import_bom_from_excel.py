...
class import_bom_from_excel_wizard(osv.osv_memory):
    ...
    def import_bom_from_excel(self, cr, uid, ids, context=None):
        ...
        bom_ids = self.pool.get('bom.converter').create_bom_from_excel_file_path(cr, uid, file_path=self._file_path)
...
class BomConverter(osv.osv):
    ...
    def find_create_product_id_from_sku(self, product_sku):
        ...
        return {'product_id':product.id,'product_tmpl_id':product.product_tmpl_id.id,'name':product.name}
    ...
    def update_bom_lines(self, bom_update_header):
        ...
        for line in bom_update_lines:
            ...
            for excel_line in excel_bom_lines:
                updated_bom_lines.append(self.update_one_bom_line(excel_line))#根据产品名称，单位名称更新产品ID,单位ID
    ...
    def update_one_bom_line(self, excel_line):
        ...
        product = self.find_create_product_id_from_sku(excel_line[2]['product_id'])
        ...
        for row in self.excel_data:
            ...
            # u'\u8f85\u6599'    辅料     u'\u4e3b\u6599'  主料
                if product_id_cur and row[3] and row[4]:
                    bom_line = (0,0,{'product_id':row[3].strip(), 'product_uos_qty':row[4],'product_qty': row[4],})
                    if not vals.get(product_id_cur,False):
                        val = {product_id_cur: {'name': row[0],'product_tmpl_id': product_id_cur,'product_id': product_id_cur,
                        'product_qty':1,'bom_line_ids':[bom_line],'bom_line2_ids':[],}
                        }
                ...
                if product_id_cur and row[3] and row[4]:
                    ...
                    if not vals.get(product_id_cur,False):
                        val = {product_id_cur: {'name': row[0],'product_tmpl_id': product_id_cur,'product_id': product_id_cur,
                        'product_qty':1,'bom_line_ids':[],'bom_line2_ids':[bom_line]}
                        }
