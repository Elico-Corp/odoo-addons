    ...
    instance.web.WebClient.include({
        ...
        get_data: function(){
            ...
            $('#oe_main_menu_navbar').find('.oe_popup_header_click').on('click', function(e) {
                form_ids = self.alive(new instance.web.Model('popup.reminder').call('get_model_name',[e.currentTarget.attributes.value.value]));
            ...
            $('#oe_main_menu_navbar').find('.oe_popup_record_click').on('click', function(e) {
                ...
                btn_data = btn_txt.split('//')
                form_ids = self.alive(new instance.web.Model('popup.reminder').call('get_form_data',[btn_data[0]]));
                form_ids.then(function(response) {
                    instance.webclient.action_manager.do_action({
                        res_model: response,
                        type: 'ir.actions.act_window',
        ...
        get_popupdata: function(){
            var self = this;
            record_header_data = self.alive(new instance.web.Model("popup.reminder").call("set_record_header"));
            record_header_data.then(function(header) {
                self.record_header = header
            })
            unique_id_data = self.alive(new instance.web.Model('popup.reminder').call('get_unique_id'));
            unique_id_data.then(function(unique) {
                self.unique_list.push(unique)
            })
            reminder_data = self.alive(new instance.web.Model("popup.reminder").call("set_notification",[false]));
            reminder_data.then(function(response) {
                self.popup_reminder_data = response
                self.length = 0
                $.each(self.popup_reminder_data, function(key, value) {
                    color_data = self.alive(new instance.web.Model('popup.reminder').call('get_color_name',[key]));
