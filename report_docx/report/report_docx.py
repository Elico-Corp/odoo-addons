...
class ReportDocx(report_sxw.rml_parse):

    @api.model
    def create(self, data):
        report_id_list = self.env['ir.actions.report.xml'].search([
            ('report_name', '=', self.name[7:])])
        if report_id_list:
            report_xml = report_id_list[0]
            self.title = report_xml.name
            if report_xml.report_type == 'docx':
                return self.create_source_docx(data)
        return super(ReportDocx, self).create(data)
    ...
    def _generate_reports(self, tmp_folder_name, datas,
        output_type, output_report, dict):
        if "pdf" == output_type:
            self._generate_pdf_reports(
                tmp_folder_name, datas,
                output_type, output_report, dict)
            return

        self._generate_doc_reports(tmp_folder_name, datas, output_type, output_report, dict)

    def _generate_pdf_reports(self, tmp_folder_name, datas, output_type, output_report, dict):
        count = 0
        for data in datas:
            self._convert_single_report(tmp_folder_name, count, data, output_type, dict)
            count = count + 1
    ...
    def _convert_single_report(
        ...
        self._convert_docx_from_template(data, tmp_folder_name,
            docx_template_name, convert_docx_file_name, dict)

        if output_type == 'pdf':
            self._convert_docx_to_pdf(tmp_folder_name,convert_docx_file_name)

            self._create_watermark_pdf(tmp_folder_name, watermark_file, dict)

            self._add_watermark_to_pdf(tmp_folder_name, watermark_file,
    ...
    def _convert_docx_from_template(self, tmp_folder_name, data,
        docx_template_name, convert_docx_file_name, dict):
        action_id = dict['template_id']
        action = self.env['ir.actions.report.xml'].browse(action_id)

        template_path = tmp_folder_name + docx_template_name
        convert_path = tmp_folder_name + convert_docx_file_name

        self._save_file(template_path, base64.b64decode(action.template_file.datas))
    ...
    def _create_watermark_pdf(self, tmp_folder_name, watermark_file, dict):
        watermark_path = tmp_folder_name + watermark_file
        watermark_string = self._get_watermark_string(dict)
        watermark_template = self._get_watermark_template(dict)

        if watermark_template:
            self._save_file(watermark_path, base64.b64decode(watermark_template))
            return
    ...
    def _get_watermark_string(self, dict):
        action_id = dict['template_id']
        action = self.env['ir.actions.report.xml'].browse(action_id)
    ...
    def _get_watermark_template(self, dict):
        action_id = dict['template_id']
        action = self.env['ir.actions.report.xml'].browse(action_id)

        return action.watermark_template.datas

    def _get_output_type(self, dict):
        action_id = dict['template_id']
        action = self.env['ir.actions.report.xml'].browse(action_id)
