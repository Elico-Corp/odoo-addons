...
from elico_multi_company_financial_statements.report import multicompany_report
from elico_multi_company_financial_statements.report.multi_company_report import *

#class multicompany_compared_report(multicompany_report):
class multicompany_compared_report(report_sxw.rml_parse):
	def __init__(self, cr, uid, name, context):
		super(multicompany_compared_report, self).__init__(cr, uid, name, context)
#to be improved
	...
	def _get_account_period_balance(self, cr, uid, context=None):
...
#future use	
	...
	def get_report_header(self, form):
		...
		if form['report_details']=='RepDet': report_details='Details'
	...
	def get_lines(self, form):
		print "form in get_lines="+str(form)
		...
		print "strfilter1="+str(strfilter)
		result = []
		level = 1
...
#10 Account_account.name \
		...
		if account_list!=[]:
			...
			for account in account_list:
				...
				bal_per1 = self._get_account_period_balance(self.cr, self.uid, ctx)*sign
				bal_budg = 0
				bal_per2 = 0 
				if compared:
					...
					bal_per2 = self._get_account_period_balance(self.cr, self.uid, ctx)*sign

	
				#root of chart of account
				if account[9]== '0':
					continue
				#print "PREV_TOTAL="+str(prev_totals)				
				totals = {
					'level1':       account[1],
						...
						if strfield:
							if prev_totals[strfield]!= totals[strfield ]:
								res2 = {
									...
									'total':       True,    
									'type':        str(i),
									'code':        (strfield=='level1')*'Grand Total' or (strfield=='level2')*'Total' or (strfield=='user_type')*'Sub-total' or 
											(strfield=='current')*'Sub-Total Current' or (strfield=='operating')*'Sub-Total Operating' ,
...
report_sxw.report_sxw('report.elico_multi_company_financial_statements.multicompany_compared_report', 
	'account.account', 
	'addons/elico_multi_company_financial_statements/report/multicompany_compared_report.rml', 
	parser=multicompany_compared_report, 
	header=False)
