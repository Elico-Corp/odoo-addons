...
class task(osv.osv):
    ...
    def _callback_prefix_name(self, cr, uid, ids, field_name, arg, context=None):
        res = {}

        for task_id in self.browse(cr, uid, ids, context=context):
            if task_id:
                partner_id = task_id and task_id.project_id and task_id.project_id.partner_id
    ...
    def onchange_project(self, cr, uid, ids, project_id):
        ...
        if project_id and 'value' in res:
            ...
            res['value'].update({'phase_id': project[0] and project[0].phase_ids and project[0].phase_ids[0].id or False})
            print res
        return res
    
    
    def _task_to_update_after_project_change(self, cr, uid, ids, fields=None, arg=None, context=None):
        if type(ids) != type([]):
            ids = [ids]
        return self.pool.get('project.task').search(cr, uid, [('project_id', 'in', ids)]) or []
    
    
    _store_prefix_name = {
        'project.task': (lambda self, cr, uid, ids, context: ids, ['code_gap','name'], 10),
        'project.project': (_task_to_update_after_project_change, ['partner_id'], 10),
    }
    
    _columns = {
        'prefix_name': fields.function(_callback_prefix_name, store=_store_prefix_name, string='Task Name with Prefix', type='char', help="This field is computed automatically  with name and project ."),
        'prefix': fields.function(_callback_prefix, string='Task Name Prefix', type='char', help="Prefix for task name"),
    }
    _defaults = {
        'planned_hours': 4,
    }
    ...
    def do_open(self, cr, uid, ids, context={}):
        ...
        vals['type_id']= task_id.project_id and task_id.project_id.type_ids and task_id.project_id.type_ids[0].id
    ...
    def action_close(self, cr, uid, ids, context=None):
        ...
        #vals['type_id']= task_id.project_id and task_id.project_id.type_ids and task_id.project_id.type_ids[-1].id
        self.write(cr, uid, [task_id.id],vals, context=context)
