...
<form string="Convert To Opportunity (Purchasing)">
    <field name="name"/>
    <newline/>
...
case_fields = {
    'name': {'type':'char', 'size':64, 'string':'Opportunity (Purchasing) Summary'},
...
class make_opportunity(wizard.interface):
    ...
    def _makeOrder(self, cr, uid, data, context):
        ...
        id = section_obj.search(cr, uid, [('code','=','opporp')], context=context)
        if not id:
            raise wizard.except_wizard(_('Error !'),
                _('You did not installed the opportunities tracking when you configured the crm_configuration_purchasing module.' \
                  '\nI can not convert the lead to an opportunity, you must create a section with the code \'opporp\'.'
                  ))
        id = id[0]

        id2 = data_obj._get_id(cr, uid, 'crm_configuration_purchasing', 'crm_case_form_view_opporp')
        if id2:
    ...
    states = {
        'init': {
            ...
            'result': {'type': 'form', 'arch': case_form, 'fields': case_fields,
                'state' : [('end', 'Cancel', 'gtk-cancel'),('confirm', 'Create Opportunity (Purchasing)', 'gtk-go-forward')]}
        },
        'confirm': {
            'actions': [],
...
make_opportunity('crm.case.opportunity_setp')
