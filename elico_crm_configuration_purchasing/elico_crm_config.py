...
class srm_menu_config_wizard(osv.osv_memory):
    ...
    def action_create(self, cr, uid, ids, context=None):
        ...
        modid = module_proxy.search(cr, uid, [('name', '=', 'elico_crm_configuration_purchasing')])
        ...
        for section in ['lead_purchasing', 'opportunity_purchasing']:
            ...
            for fname in lst:
                ...
                try:
                    fp = tools.file_open(os.path.join('elico_crm_configuration_purchasing', file_name))
                except IOError, e:
                    fp = None
                if fp:
                    tools.convert_xml_import(cr, 'elico_crm_configuration_purchasing', fp, idref, 'init', noupdate=True)
        cr.commit()
        modobj = self.pool.get('ir.module.module')
        modids = modobj.search(cr, uid, [('name', '=', 'elico_crm_configuration_purchasing')])
        modobj.update_translations(cr, 1, modids, None)

        if res['document_ics']:
            ids = module_proxy.search(cr, uid, [('name', '=', 'elico_document_ics_purchasing')])
