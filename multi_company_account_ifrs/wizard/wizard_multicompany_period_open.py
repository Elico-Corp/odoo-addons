...
def _data_save(self, cr, uid, data, context):
    ...
    if not data['form']['sure']:
        raise wizard.except_wizard(_('UserError'), _('Process canceled, please check the box !'))
...
#                ids = pool.get('account.move.line').search(cr, uid, [('id','=',tmpid)]) 
...
class wizard_multicompany_period_open(wizard.interface):
    states = {
        'init': {
            'actions': [_data_load],
...
wizard_multicompany_period_open('multi_company_account_ifrs.multicompany.period.open')
