...
class multicompany_account_report(report_sxw.rml_parse):
	def __init__(self, cr, uid, name, context):
		super(multicompany_account_report, self).__init__(cr, uid, name, context)
...
#future use	
	...
	def get_report_header(self, form):
		result = []
		clause=')'.join(str('('.join(str(form['periods1'][0][2]).split('['))).split(']'))
		self.cr.execute('SELECT name FROM "account_period" WHERE id in '+clause+' ORDER BY name', ) 
		periods1=''
		for x in self.cr.fetchall():
			periods1= periods1 +', '*(periods1!='')+ str(x[0])	
		periods2 =    []
		if form['periods2'][0][2]!=[]:
			clause=')'.join(str('('.join(str(form['periods2'][0][2]).split('['))).split(']'))
			self.cr.execute('SELECT name FROM "account_period" WHERE id in '+clause+' ORDER BY name', ) 
			periods2=''
			for x in self.cr.fetchall():
	...
	def get_lines(self, form):
		...
		periods1 = form['periods1'][0][2]
		periods2 = form['periods2'][0][2]
		compared = periods2 !=[]
...
#10 Account_account.name \
		...
		if account_list!=[]:
			...
			for account in account_list:
				...
				ctx['periods'] = periods1 
				ctx['fiscalyear'] = fiscalyear1 
				bal_per1 = self._get_account_period_balance(self.cr, self.uid, ctx)*account[6]
				bal_budg = 0
				bal_per2 = 0 
				if compared:
					bal_budg = self._get_account_period_budget(self.cr, self.uid, ctx)*account[6]
					ctx['periods'] = periods2 
					ctx['fiscalyear'] = fiscalyear2 
					bal_per2 = self._get_account_period_balance(self.cr, self.uid, ctx)*account[6]
...
report_sxw.report_sxw('report.multi_company_account_ifrs.multicompany.account.report', 'account.account', 'addons/multi_company_account_ifrs/report/multicompany_account_report.rml', parser=multicompany_account_report, header=False)
